...
from memryx.runtime.benchmark import _SystemInfo
...
class Accl(ABC):
    def __init__(self, dfp, group_id=0,  **kwargs):
        ...
        if not isinstance(model_id, int) or model_id < 0:
            raise TypeError("Model ID must be an non-negative integer")

        MAX_MODEL_ID = 32
        if model_id not in range(MAX_MODEL_ID):
            raise ValueError("Model ID must be in the range [0,31]")

        if not isinstance(group_id, int) or group_id < 0:
            raise TypeError("Group ID must be an non-negative integer")

        self._dfp = dfp
        self._sys_info = self._parse_dfp(dfp)
        print(f'Chip Gen: {self._sys_info.gen}')
        self._model_id = model_id
        self._group_id = group_id
        self._chip_gen = self._sys_info.gen # add check to compare DFP chip gen with H/W when possible
    ...
    def _configure(self, dfp, inport_mapping={}, outport_mapping={}):
        ...
        for idx_pair, mapping in inport_mapping.items():
            ...
            for k in ['model_index', 'layer_name']:
                self._sys_info.input_ports[port][k] = mapping[k]
        for idx_pair, mapping in outport_mapping.items():
            mpu, port = idx_pair
            for k in ['model_index', 'layer_name']:
                self._sys_info.output_ports[port][k] = mapping[k]

        self._model_idx_to_out_info = defaultdict(lambda : defaultdict(list))
        self._outlayer_to_port_idx = {}
        for i, info in self._sys_info.output_ports.items():
        ...
        for i, info in self._sys_info.input_ports.items():
            row,col,z,ch = info['shape']
            if info['format'] in ['gbfloat80', 'bf16', 'float'] or info['data_range_enabled']:
                dtype = np.float32
            else:
                dtype = np.uint8
            info['dtype'] = dtype
    ...
    def _parse_dfp(self, dfp):
        ...
        if isinstance(dfp, (str, Path)):
            ...
            with open(self._dfp_path, 'rb') as f:
                self._dfp = f.read()
        else:
            self._dfp = dfp
            self._tmp_dfp_file = tempfile.NamedTemporaryFile(dir=os.getcwd(), suffix='.dfp')
            self._tmp_dfp_file.write(dfp)
            self._dfp_path = self._tmp_dfp_file.name

        sys_info = _SystemInfo(self._dfp_path)
        ver = sys_info.dfp_version
        if ver == 'legacy' or int(ver) < 4:
            raise RuntimeError(f"Unsupported DFP version: {ver}, please recompile to the latest version")
        sys_info.input_ports = {p: info for p, info in sys_info.input_ports.items() if info['active']}
        sys_info.output_ports = {p: info for p, info in sys_info.output_ports.items() if info['active']}
        return sys_info

    def _init(self, model_id, group_id, chip_gen, model_idx=0, model_type=3):
        # clean up context from existing instance
        memryx.mxa.close(model_id)
        time.sleep(0.01)
        memryx.mxa.unlock(group_id)

        if memryx.mxa.lock(group_id):
            raise RuntimeError(f'Error getting lock on the accelerator')
        if memryx.mxa.open(model_id, group_id, chip_gen):
        ...
        hw_mpus = memryx.mxa.chip_count(self._model_id, 0)
        ...
        if memryx.mxa.download(model_id, self._dfp_path, model_idx, model_type):
            raise RuntimeError(f'Error loading DFP, try resetting the board')
        if memryx.mxa.set_stream_enable(model_id, 0):
    ...
    def _load_dfp(self, model_id, group_id, dfp_path, model_idx=0, model_type=3):
        ...
        err = memryx.mxa.lock(group_id)
        if err:
            self._printl(f'Error getting lock on memryx.mxa: {err}')
        err = memryx.mxa.open(model_id, group_id, self._chip_gen)
        if err:
            self._printl(f'Error opening memryx.mxa: {err}')
        err = memryx.mxa.download(model_id, dfp_path, model_idx, model_type)
        if err:
            self._printl(f'Error loading dfp: {err}')
        err = memryx.mxa.set_stream_enable(model_id, 0)
        if err:
    ...
    def shutdown(self):
        ...
        """
        time.sleep(0.05)
        memryx.mxa.close(self._model_id)
        time.sleep(0.01)
        memryx.mxa.unlock(self._group_id)
        if self._tmp_dfp_file:
            self._tmp_dfp_file.close()
...
class AcclModelInput(ABC):
    ...
    def set_preprocessing(self, model):
        ...
        from memryx.utilities.model_loader import ModelLoader

        if model is None:
            raise RuntimeError("pre-processing model is None")
        cfg = dict(raw_model=True)
        if isinstance(model, str):
            self._pre_model = ModelLoader().load(model, cfg.copy())
        else:
            self._pre_model = ModelLoader().from_memory(model, cfg.copy())
...
class AcclModelOutput(ABC):
    ...
    def set_postprocessing(self, model):
        ...
        cfg = dict(raw_model=True)
        if isinstance(model, str):
            self._post_model = ModelLoader().load(model, cfg.copy())
        else:
            self._post_model = ModelLoader().from_memory(model, cfg.copy())
...
class SyncAcclModelInput(AcclModelInput):
    ...
    def send(self, data, port_idx, timeout=None):
        ...
        err = memryx.mxa.stream_ifmap(self._model_id, p, data.copy(), timeout=timeout)
...
class SyncAcclModelOutput(AcclModelOutput):
    ...
    def receive(self, port_idx, timeout=None):
        ...
        p = self._info['ports'][port_idx]
        err = memryx.mxa.stream_ofmap(self._model_id, p, ofmap, timeout=timeout)
        if err:
...
def graceful_shutdown(func):
    def exc_handling_wrapper(*args, **kwargs):
        try:
            ...
            self = args[0]
            self.stop()
            if self == self._model.input:
                self._model.output.stop()
            else:
                self._model.input.stop()

            for model in self._accl.models:
                if model == self._model:
                    continue
                model.stop() # might block forever if app code's in/out function is blocked on sth. like a queue.get()
...
class AsyncAcclModelInput(AcclModelInput):
    ...
    def connect(self, callback):
        ...
        if self._thread is not None:
            self.stop()
            self._stop = threading.Event()
        thread_name = f'Model {self._accl.models.index(self._model)} input function'
        self._thread = threading.Thread(
                target=self._worker,
                args=(callback, self._stop),
                name=thread_name,
                )
        self._thread.start()
    ...
    @graceful_shutdown
    def _worker(self, callback, stop_event):
        iterator = None
        while not stop_event.is_set():
            try:
                if iterator is not None:
                    input_frames = next(iterator)
                else:
                    obj = callback()
                    if type(obj).__name__ == 'generator':
                        iterator = obj
                        input_frames = next(obj)
                    else:
                        input_frames = obj
            except StopIteration:
                input_frames = None
            except Exception as e:
                print('Model input stream terminated due to an exception in the callback function:')
                raise e from None

            if input_frames is None:
                stop_event.set()
                break
            mpu_input_order = self._info['layers']
            try:
                input_frames = self._pre_model.predict_for_mxa(input_frames, format_output=True, output_order=mpu_input_order)
            except Exception as e:
                raise RuntimeError(
            ...
            for f, p, shape, dtype in zip(input_frames, self._info['ports'], self._info['shapes'], self._info['dtypes']):
                ...
                memryx.mxa.stream_ifmap(self._model_id, p, f.copy())
...
class AsyncAcclModelOutput(AcclModelOutput):
    ...
    def connect(self, callback):
        ...
        if self._thread is not None:
            self.stop()
            self._stop = threading.Event()
        thread_name = f'Model {self._accl.models.index(self._model)} output function'
        self._thread = threading.Thread(
                target=self._worker,
                args=(callback, self._stop, self._model.input),
                name=thread_name,
                )
        self._thread.start()
    ...
    @graceful_shutdown
    def _worker(self, callback, stop_event, model_input):
        ...
        while not stop_event.is_set():
            ...
            check_exit = False
            while idx < len(self._info['ports']):
                p = self._info['ports'][idx]
                shape = self._info['shapes'][idx]
                dtype = self._info['dtypes'][idx]
                ofmap = np.zeros(shape).astype(dtype)
                err = memryx.mxa.stream_ofmap(self._model_id, p, ofmap, timeout=500)
            ...
            try:
                ...
                print('Model output processing terminated due to an exception in the callback function:')
                raise e from None
...
class InputTaskPool:
    ...
    def _worker_target(self):
        while not self._stop_event.is_set():
            ...
            task.model.input.put(input_frames, task.stream)
    ...
    def _execute_task(self, task):
        try:
            obj = task.stream.input_callback(task.stream.index)
            ...
            logger.critical(f'Model {task.model.name()} input stream {stream_idx} terminated due to an exception in the callback function:')
            if isinstance(e, AssertionError):
                logger.critical("AssertionError")
            else:
...
class MultiStreamAsyncAcclModelInput(AcclModelInput):
    def __init__(self, model_id, info, accl, model):
        ...
        self._call_counter = 0
        self._call_counter_lock = threading.Lock()
        self._merge_queue = queue.Queue() # TODO: come up with a max-size based on frame memory consumption
        self._output_stream_queue = queue.Queue()
        self._pre_model = _PassThroughModel()
        self._thread = threading.Thread(
                target=self._worker,
                args=(self._merge_queue, self._output_stream_queue, self._stop_event),
    ...
    def put(self, input_frames, stream):
        self._merge_queue.put((input_frames, stream))
    ...
    @graceful_shutdown
    def _worker(self, merge_queue, output_stream_queue, stop_event):
        done_count = 0
        while not stop_event.is_set():
            try:
...
class MultiStreamAsyncAcclModelOutput(AcclModelOutput):
    ...
    def _stream_ofmaps(self, outputs, outputs_by_name, timeout=500):
        # returns True if streamed data
        for idx, p in enumerate(self._info['ports']):
            shape = self._info['shapes'][idx]
            dtype = self._info['dtypes'][idx]
            ofmap = np.zeros(shape).astype(dtype)
            err = memryx.mxa.stream_ofmap(self._model_id, p, ofmap, timeout)
            if err: # check specifically for timeout and not others
                logger.debug(f"Model {self._model.name()} stream outputs timed out")
                return False
            outputs.append(ofmap)
            outputs_by_name[self._info['layers'][idx]] = ofmap
        return True
    ...
    @graceful_shutdown
    def _worker(self, stop_event, model):
        outputs = []
        outputs_by_name = {}

        while not stop_event.is_set():
            if model.input.stopped() and self._all_frames_processed():
                logger.debug(f"Model {self._model.name()} sent all outputs")
                return

            streamed = self._stream_ofmaps(outputs, outputs_by_name)
            if not streamed:
                continue
        ...
        while self._stream_ofmaps(outputs, outputs_by_name):
            outptus = []
            outputs_by_name = {}
